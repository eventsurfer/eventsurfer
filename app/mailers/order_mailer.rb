...
class OrderMailer < ApplicationMailer
  ...
  def entry_order(order)
    ...
    attachments["order.pdf"] = WickedPdf.new.pdf_from_string(render_to_string(pdf: "order",
                                                                              template: "layouts/pdf/order",
                                                                              layout: "pdf/general.erb"),
                                                             viewport_size: "1280x1024", page_size: "A4",
                                                             width: 1280, height: 1024)
    mail(subject: "Your order confirmation", to: "<" + @user.email.to_s + ">", template_path: "layouts/mailer",
         template_name: "entry_order")
  end
  ...
  def sendTickets(order)
    ...
    @qrs = tmp
    @user = User.find(@order.user_id)
    mail(subject: "Your tickets", to: "<" + @user.email.to_s + ">", template_path: "layouts/mailer",
         template_name: "sendTickets")
  end
end
